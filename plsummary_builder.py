...
def _parse_tax_summary_block(rows: list[list]) -> dict:
    ...
    data = {
        ...
        "realized_long_term_gain": None,
        "realized_short_term_gain": None,
        "realized_gain": None,  # fallback when long/short not detectable
        "taxable_income": None,
    }
    for r in (rows or []):
        r = r if isinstance(r, (list, tuple)) else [r]
        ...
        if "dividend" in label:
            ...
            if "long" in label and "term" in label:
                data["realized_long_term_gain"] = v or data.get("realized_long_term_gain")
            elif "short" in label and "term" in label:
                data["realized_short_term_gain"] = v or data.get("realized_short_term_gain")
            else:
...
def build_plsummary_jpm_sheet(
    ...
    # Depending on extraction/TOC grouping, the portfolio activity table may live under "Portfolio Activity" or "Overview".
    port_rows = by_target.get("Portfolio Activity", []) or by_target.get("Overview", [])
    port_block = port_rows[0][1] if port_rows else []
    # Likewise, tax tables may be grouped under "Tax Summary" or "US Tax Summary".
    tax_rows = by_target.get("Tax Summary", []) or by_target.get("US Tax Summary", [])
    tax_block = []
    ...
    # === Apply rule-based overrides from the meeting ===
    # Account id is required to pick the right time-series block.
    acct_id = acc["accounts"][0]["id"] if acc["accounts"] else "E79271004"

    # Cash (Cash Activity -> closing balance)
    cash_from_pdf = extract_cash_closing_balance(by_target)
    if cash_from_pdf is not None:
        cash_equiv = cash_from_pdf

    # Purchases & Sales accrued interest sign rule
    purchases_accrued_interest = extract_purchases_sales_accrued_interest(by_target)
    interest_paid_on_purchases, interest_income_from_purchases = map_interest_from_purchases_sales(
        purchases_accrued_interest
    )
    if purchases_accrued_interest is None:
        # fallback: keep tax summary interest income if purchases & sales total wasn't extractable
        interest_paid_on_purchases = 0.0
        interest_income_from_purchases = interest_income or 0.0

    # Time-series deltas for the same account block
    change_in_unrealized = extract_unrealized_change_for_account(by_target, acct_id)
    change_in_accrued_interest = extract_change_in_accrued_interest_for_account(by_target, acct_id)
    change_in_accrued_dividend = extract_change_in_accrued_dividend_for_account(by_target, acct_id)
    # === End overrides ===
    ...
    row += 1
    # Line items (QB sample order). Populate values we can confirm from PDF extraction:
    # - Change in Accrued Dividend / Interest: from Change in Dividend / Change in Interest sheets
    # - Change in unrealized gain/loss: from Unrealized sheet delta
    # - Dividend Income: from Tax Summary
    # - Interest Paid on Purchases / Interest Income: from Purchases & Sales sign rule
    line_items = [
        "Change in Accrued Dividend",
    ...
    realized_long_term = tax.get("realized_long_term_gain") or tax.get("realized_gain")
    realized_short_term = tax.get("realized_short_term_gain") or None

    for label in line_items:
        ws.cell(row, 1, label)
        val = None
        if label == "Change in Accrued Dividend":
            val = change_in_accrued_dividend
        elif label == "Change in Accrued Interest":
            val = change_in_accrued_interest
        elif label == "Change in unrealized gain/loss":
            val = change_in_unrealized
        elif label == "Dividend Income" and dividend_income is not None:
            val = dividend_income
        elif label == "Interest Paid on Purchases":
            val = interest_paid_on_purchases
        elif label == "Interest Income":
            val = interest_income_from_purchases
        elif label == "Long term realized gain/loss":
            val = realized_long_term
        elif label == "Short term realized gain/loss":
            val = realized_short_term

        ws.cell(row, 2, round(float(val), 2) if val is not None else 0)
        row += 1
    row += 1
